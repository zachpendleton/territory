(ns territory.game-test
  (:require [clojure.test :refer :all]
            [territory.game :refer :all]))

(deftest start-game-test
  (let [new-game (new-game "abc123" 2 {:width 5 :height 5})]
...
(deftest add-player-test
  (let [game (new-game "abc123" 2 {:width 5 :height 5})
        player-a {:id "a"}
        player-b {:id "b"}
        player-c {:id "c"}]

    (is (= 1 (count (:players (add-player game player-a))))
        "adds the player")
    (is (= 1 (:awaiting (meta (:players (add-player game player-a)))))
        "decrements the awaited-players count")

    (is (= 1 (count (:players (-> game (add-player player-a) (add-player player-a)))))
        "doesn't add the same player more than once")
    (is (= 1 (:awaiting (meta (:players (-> game (add-player player-a) (add-player player-a))))))
        "doesn't adjust the awaited-players count when given a duplicate player")

    (let [full-game (-> game (add-player player-a) (add-player player-b))]
      (is (= full-game (add-player full-game player-c))
          "ignores extra players by default")

      (binding [handle-extra-player #(hash-map :game %1 :player %2)]
        (is (= full-game (:game (add-player full-game player-c)))
            "custom handler receives the original game")
        (is (= player-c (:player (add-player full-game player-c)))
            "custom handler receives the extra player")))))

(deftest deal-hands-test
  (let [game (assoc (new-game "abc123" 2 {:width 5 :height 5})
                    :players (into [] (map atom [{:id "a"} {:id "b"}]))
                    :draw [{:row 1 :col 1} {:row 1 :col 2} {:row 1 :col 3}
                           {:row 2 :col 1} {:row 2 :col 2} {:row 2 :col 3}])]
    (is (= [[{:row 1 :col 1} {:row 1 :col 2} {:row 1 :col 3}]
            [{:row 2 :col 1} {:row 2 :col 2} {:row 2 :col 3}]]
           (map :hand (:players (deal-hands game 3))))
        "deals hands to each player")
    (is (empty? (:draw (deal-hands game 3)))
        "removes dealt cards from draw pile")

    (is (= game (deal-hands game 6))
        "deals no cards when hand size is too big by default")

    (binding [handle-not-enough-cards #(hash-map :game %1 :hand-size %2)]
      (is (= game (:game (deal-hands game 6)))
          "custom handler receives the original game")
      (is (= 6 (:hand-size (deal-hands game 6)))
          "custom handler receives the specified hand size"))))

(deftest current-player-test
  (let [game (assoc (new-game "abc123" 3 {:width 3 :height 3})
                    :players (into [] (map atom [{:id "a"} {:id "b"} {:id "c"}])))]
    (is (= {:id "a"} @(current-player game))
        "new game has first player as current")
    (is (= {:id "c"} @(current-player (assoc game :played-tiles [:tile-1 :tile-2])))
        "game with played tiles returns next player to play")
    (is (= {:id "a"} @(current-player (assoc game :played-tiles [:tile-1 :tile-2 :tile-3])))
...
(deftest pick-winner-test
  (testing "returns a single winning army regardless of favor"
    (is (= {:player 1 :cells [1 2 3]}
           (pick-winner [{:player 1 :cells [1 2 3]}
                               {:player 2 :cells [1]}]
                              2))))
